...
const config = {
    ...
    appCode: '2735a3f25b944d83abb05c374ccc0639',
    appKey: '203881056',
    appSecret: 'pwvKnKTF1UJ2HRSz4aelL93bg7bhljzD'
}
...
app.get('/getCovid19Data', (req, res) => {
    readFile(path.join(__dirname, '../data.json')).then(dataStr => {
        ...
        data.provinceArray.forEach(province => {
            province['Id'] = count;
            count++;
            province.cityArray.forEach(city => {
                city['Id'] = count;
                count++;
            })
...
app.get('/cq', (req, res) => {
    readFile(path.join(__dirname, '../cq.json')).then(dataStr => {
        res.json({
            success: true,
            message: '操作成功',
            data: JSON.parse(dataStr)
...
app.get('/worldCovid19Data', (req, res) => {
    readFile(path.join(__dirname, '../world.json')).then(dataStr => {
        ...
        data.data.forEach(item => {
            confirm += item['confirm'] == null ? 0 : item['confirm']
            dead += item['dead'] == null ? 0 : item['dead'];
            heal += item['heal'] == null ? 0 : item['heal'];
            nowConfirm += item['nowConfirm'] == null ? 0 : item['nowConfirm'];
            confirmAdd += item['confirmAdd'] == null ? 0 : item['confirmAdd'];
            suspect += item['suspect'] == null ? 0 : item['suspect'];
        })
        res.json({
            success: true,
            message: '操作成功',
